...
After a successful review, read REPORT.md inside `.reviews/<session-id>/` and present findings grouped by severity (critical → high → medium → low → info). Include the session ID so the user can reference findings with `/fix`.
